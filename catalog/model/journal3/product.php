...
class ModelJournal3Product extends Model {
	...
	public function formatProduct_old26062025($product) {
	    $this->load->model('tool/image');

	    $product['thumb'] = $product['image']
	        ? $this->model_tool_image->resize($product['image'], 500, 500)
	...
	public function formatProduct($product) {
	    ...
	    $product['rating'] = $this->config->get('config_review_status') ? $product['rating'] : false;
	    $product['href'] = $this->url->link('product/product', 'product_id=' . $product['product_id']);
	   // echo "<pre>";print_r($this->settings);exit();
	    $product['stat1'] = defined('JOURNAL3_ACTIVE') ? $this->journal3->productStat($product, 'brand') : null;
	    $product['stat2'] = defined('JOURNAL3_ACTIVE') ? $this->journal3->productStat($product, Arr::get($this->settings, 'moduleProductStat2')) : null;
	    
	    $product['labels'] = defined('JOURNAL3_ACTIVE') ? $this->journal3->productLabels($product, $price, $special) : null;
		$product['extra_buttons'] = defined('JOURNAL3_ACTIVE') ? $this->journal3->productExtraButton($product, $price, $special) : null;
		$product['date_end'] = defined('JOURNAL3_ACTIVE') ? $this->journal3->productCountdown($product) : null;
		$product['price_value'] = defined('JOURNAL3_ACTIVE') ? ($product['special'] ? $product['special'] > 0 : $product['price'] > 0) : null;
	...
	public function getProductOptionValues($product_id, $product_options) {
		...
		return $this->db->query($sql)->rows;
			...
			// Step 1: Get category IDs assigned to the current product
			$query = $this->db->query("
				SELECT category_id 
			...
			// Step 2: Get subcategory IDs of those categories
			$subcategory_ids = [];
			...
			if ($category_ids) {
				...
				foreach ($sub_query->rows as $row) {
					$subcategory_ids[] = (int)$row['category_id'];
				}
			}

			if (empty($subcategory_ids)) {
				return array(); // No subcategories found
			}

			$subcategory_ids_sql = implode(',', $subcategory_ids);

			// Step 3: Get random products from those subcategories
			$sql = "
				SELECT DISTINCT p.* 
				...
				WHERE 
					p2c.category_id IN ($subcategory_ids_sql)
		...
		public function getRelatedProducts_10($product_id, $limit) {
		// Step 1: Get subcategory ID of the product
		$query = $this->db->query("
			SELECT category_id 
			FROM `" . DB_PREFIX . "product_to_category` 
			WHERE product_id = '" . (int)$product_id . "'
		...
		// Use only subcategory ID(s) directly assigned to the product
		$subcategory_ids = [];
		foreach ($query->rows as $row) {
			$subcategory_ids[] = (int)$row['category_id'];
		}
		...
		// Step 2: Get related products from those subcategories
		$sql = "
			SELECT DISTINCT p.* 
			FROM `" . DB_PREFIX . "product` p
			LEFT JOIN `" . DB_PREFIX . "product_to_category` p2c ON (p.product_id = p2c.product_id)
	...
	public function getRelatedProducts_old($product_id, $limit) {
		$product_ids = $this->getProductIds($product_id);
