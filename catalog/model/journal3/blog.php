...
class ModelJournal3Blog extends Model {
	...
	//  Set MySQL timezone to India time (IST)
	...
	//  Insert comment with correct time
	$sql = "
	...
	//  Return the inserted comment with correct time
	return $this->getComment($this->db->getLastId());
}


	public function createComment_old($data) {
		$parent_id = (int)Arr::get($data, 'parent_id', 0);
		$post_id = (int)Arr::get($data, 'post_id', 0);
		$name = Arr::get($data, 'name', '');
		$email = Arr::get($data, 'email', '');
