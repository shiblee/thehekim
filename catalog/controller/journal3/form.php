...
class ControllerJournal3Form extends ModuleController {
	...
	if (defined('JOURNAL3_FORM_LOG') && JOURNAL3_FORM_LOG) {
		...
		file_put_contents($file, json_encode($logData, JSON_PRETTY_PRINT) . PHP_EOL . PHP_EOL, FILE_APPEND);
	}

	try {
		if (!Request::isAjax()) {
			$this->renderJson(self::SUCCESS, array('message' => 'Success!'));
			return;
		}
		...
		if (!$this->index(array('module_id' => $module_id, 'module_type' => 'form', 'return_settings' => true))) {
		...
		foreach ($this->settings['items'] as $index => $item) {
			...
			if ($item['type'] === 'telephone' || stripos($item['label'], 'phone') !== false) {
				$phone = $value;
			}
			if ($item['type'] === 'select' || stripos($item['label'], 'enquiry') !== false) {
				$enquiryType = $value;
			}
			if ($item['type'] === 'textarea' || stripos($item['label'], 'message') !== false) {
				$messageText = $value;
			}
		...
		if ($errors) {
			...
			$params = array(
				...
				'subject' => 'New Contact Us Form Submission - ' . $enquiryType,
			...
			if (isset($data['email'])) {
				$userMail = new Mail();
				$userMail->protocol = $this->config->get('config_mail_protocol');
				$userMail->parameter = $this->config->get('config_mail_parameter');
				$userMail->smtp_hostname = $this->config->get('config_mail_smtp_hostname');
				$userMail->smtp_username = $this->config->get('config_mail_smtp_username');
				$userMail->smtp_password = html_entity_decode($this->config->get('config_mail_smtp_password'), ENT_QUOTES, 'UTF-8');
				$userMail->smtp_port = $this->config->get('config_mail_smtp_port');
				$userMail->smtp_timeout = $this->config->get('config_mail_smtp_timeout');

				$userMail->setTo($data['email']);
				$userMail->setFrom($this->config->get('config_email'));
				$userMail->setSender($this->config->get('config_name'));
				$userMail->setSubject('Your Enquiry has been Submitted Successfully');

				$userMessage = "Dear {$data['name']},\n\n";
				$userMessage .= "Thank you for contacting theHekim!\n\n";
				$userMessage .= "We’ve received your enquiry and it has been successfully submitted. Our team is currently reviewing your request and will get back to you within 24–48 working hours.\n\n";
				$userMessage .= "Here are the details of your submission:\n";
				$userMessage .= "Type of Enquiry: " . $enquiryType . "\n";
				$userMessage .= "Message:\n" . $messageText . "\n\n";
				$userMessage .= "If you need to update your enquiry or have any additional questions, please feel free to reply to this email.\n\n";
				$userMessage .= "Thank you for choosing theHekim. We look forward to assisting you soon.\n\n";
				$userMessage .= "Best regards,\nCustomer Support Team\ntheHekim";

				$userMail->setText($userMessage);
				$userMail->send();
			}
