...
class ControllerJournal3ProductTabs extends ModuleController {
	...
	public function index($args) {
		...
		$cache = $this->journal3->cache->get($this->module_type . '.' . $this->module_id);
	...
	protected function parseGeneralSettings($parser, $index) {
		...
		switch ($parser->getSetting('contentType')) {
			...
			case 'reviews':
				...
				$data['content'] = $this->productContent('main_ingredients', null);
				break;

				case 'key_features':
				$data['content'] = $this->productContent('key_features', null);
				break;

				case 'dosage':
				$data['content'] = $this->productContent('dosage', null);
				break;

				case 'faq_description':
				$data['content'] = $this->productContent('faq_description', null);
				break;
	...
	private function productContent($type, $short_description_limit) {
		if (static::$PRODUCT_INFO === null) {
			...
			static::$PRODUCT_INFO['main_ingredients'] = html_entity_decode(Arr::get($product_info, 'main_ingredients') ?? '', ENT_QUOTES, 'UTF-8');

			static::$PRODUCT_INFO['key_features'] = html_entity_decode(Arr::get($product_info, 'key_features') ?? '', ENT_QUOTES, 'UTF-8');

			static::$PRODUCT_INFO['dosage'] = html_entity_decode(Arr::get($product_info, 'dosage') ?? '', ENT_QUOTES, 'UTF-8');

			static::$PRODUCT_INFO['faq_description'] = html_entity_decode(Arr::get($product_info, 'faq_description') ?? '', ENT_QUOTES, 'UTF-8');


			// attrs
			if (defined('JOURNAL3_OLD_OC3039')) {
