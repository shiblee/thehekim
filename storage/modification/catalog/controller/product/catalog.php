	...
	public function index() {
		...
		foreach ($results as $result) {
			...
			$data['products'][] = array(
				...
				'thumb'       => $image,
				'name'        => $result['name'],
				'description' => \Journal3\Utils\Str::utf8_substr(trim(strip_tags(html_entity_decode($result['description'], ENT_QUOTES, 'UTF-8'))), 0, (int)$this->journal3->themeConfig('product_description_length')) . '..',
				'price'       => $price,
				'special'     => $special,
				'tax'         => $tax,
		...
		if (isset($this->request->get['limit'])) {
            ...
            if (defined('JOURNAL3_ACTIVE')) {
                $data['journal3_filter_sort'] = \Journal3\Utils\Arr::get($this->request->get, 'sort');
                $data['journal3_filter_order'] = \Journal3\Utils\Arr::get($this->request->get, 'order');
		...
		sort($limits);
            ...
            if (defined('JOURNAL3_ACTIVE')) {
                $data['journal3_filter_limit'] = \Journal3\Utils\Arr::get($this->request->get, 'limit');
                $url .= $this->model_journal3_filter->buildFilterData($filter_data);
            }
